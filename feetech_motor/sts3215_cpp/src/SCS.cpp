...
SCS::SCS()
{
	Level = 1;//브로드캐스트 명령을 제외한 모든 명령은 응답을 반환합니다.
	u8Status = 0;
}
...
// 동기 쓰기 명령
// 서보 ID 배열, ID 배열 길이, 메모리 주소, 쓸 데이터, 쓰기 길이
...
// 읽기 명령
// 서보 ID, MemAddr(메모리 테이블 주소), 반환 데이터 nData, 데이터 길이 nLen
...
//Ping 명령; 서보 ID를 반환하거나, 타임아웃 시 -1을 반환합니다.
int	SCS::Ping(u8 ID)
{
	rFlushSCS();
